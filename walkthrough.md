...
8. **Tap the sessions icon (top-right)** → **Previous Sessions** list (state pill, duration, event/confirmed counts) → tap a session → its header, chain verification, events, **ASK AI ABOUT THIS SESSION**, and per-session Export / Delete in the toolbar menu (delete removes the audio clips and snapshots too; Evidence Lock and the live session block it). *Open full timeline* in the list's menu keeps the flat view and Evidence Lock reachable
9. **Type in query box**: "what happened before the alarm?" → get text answer ("list manual tags" also works)
9b. **Chat with a cloud model**: Settings → **Cloud AI** → pick Groq / Google AI Studio / OpenRouter, paste that provider's API key, type the model name exactly as they spell it → Save. Then **ASK AI** on the Timeline (all sessions) — or ask for a per-session chat from review. The model answers only from the session's event log; it is review tooling, not evidence
...
7. **Background capture now works, within platform rules** — while a session is armed, a foreground service (`camera|microphone`) owns every pipeline, so capture survives screen-off and leaving the app. Remaining honest caveats: the system can still kill the service under extreme memory pressure (the session is then recovered as `INTERRUPTED` — never a silently fake recording), and Android 13+ will prompt for notifications so the recording notice is visible. Evidence now lives in internal app-private storage (migrated from `Android/data` on first launch), excluded from cloud backup and device transfer, and every clip/photo carries a write-time SHA-256 verified in the detail view.
...
9. **Cloud AI chat sends session metadata to the provider you configure** — event labels, times, statuses, sources, confidences and counts (text only; no audio, no photos, no hashes). The key is stored in EncryptedSharedPreferences and used only for your requests. Answers can be wrong; they are never written into the chain. Without a configured key, chat is unavailable — the local QueryEngine needs no network.
